...
import { TemplateMetadataForm } from "@/components/survey/TemplateMetadataForm";
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        e.preventDefault();
        const next =
          selectedIndex < questions.length - 1 ? selectedIndex + 1 : 0;
        setSelectedQuestionId(questions[next]!.id);
      }
      if (e.key === "ArrowUp") {
        e.preventDefault();
        const prev =
          selectedIndex > 0 ? selectedIndex - 1 : questions.length - 1;
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="px-4 py-2 border-b border-border shrink-0 flex items-center gap-3">
        ...
        <span className="text-sm font-semibold text-foreground">
          {template.survey_title || template.name || "İsimsiz Template"}
        </span>
      </div>

      {/* 3-column layout */}
      <div className="flex flex-1 min-h-0 overflow-hidden max-h-[calc(100vh-2rem)]">
        {/* Left: 268px */}
        <div className="shrink-0 border-r border-border overflow-hidden flex flex-col w-96">
          <TemplateLeftPanel
            template={template}
            questions={questions}
        ...
        {/* Middle: flex-1 */}
        <div className="flex-1 overflow-y-auto bg-muted/20 p-8 max-h-[calc(100vh-2rem)]">
          {selectedQuestion ? (
            <QuestionForm question={selectedQuestion} index={selectedIndex} />
          ) : (
            <TemplateMetadataForm template={template} />
          )}
        </div>

        {/* Right: 296px */}
        <div className="shrink-0 border-l border-border overflow-hidden w-96 max-h-[calc(100vh-2rem)]">
          {selectedQuestion ? (
            <QuestionSettingsPanel question={selectedQuestion} />
          ) : (
            <div className="flex items-center justify-center h-full">
              <p className="text-xs text-muted-foreground text-center px-4">
                Template metadata ortada duzenlenir. Ayarlar paneli soru secince
                acilir.
              </p>
            </div>
          )}
